...
def real_main(ipJsonFileName):
    ...
    for ctm in list_ctm:
        ...
        for ext in typeExtension:
            ...
            opStr = compute_wer_cmd+" --text --mode=present ark:"+refFileExt+" ark:"+hypFileExt
            print(opStr)
            result = subprocess.run([compute_wer_cmd, "--text", "--mode=present", "ark:"+refFileExt,"ark:"+hypFileExt], capture_output=True, text=True)
            print(result)
            opStr = "WER:"+hypFile+"\nType:"+ext+"\n"+result.stdout+'\n'
            opWER.write(opStr)
            print(opStr)
